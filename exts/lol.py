...
References
----------
Variation of https://github.com/Sachaa-Thanasius/Beira/blob/main/exts/lol.py.
"""
...
GECKODRIVER = Path(__file__).parents[1].joinpath("drivers/geckodriver.exe")
...
class UpdateOPGGView(discord.ui.View):
    ...
    def __init__(self, bot: core.Bot, summoner_name_list: list[str]) -> None:
...
class LoLCog(commands.Cog, name="League of Legends"):
    ...
    @lol.command("handbook")
    async def lol_handbook(self, ctx: core.Context) -> None:
        """Display the handbook for Int Co."""

        async with aiofiles.open(Path(__file__).parents[1].joinpath("data/lol_handbook.txt")) as file:
            handbook = await file.read()

        embed = discord.Embed(title="EMPLOYEE HANDBOOK", description=handbook)
